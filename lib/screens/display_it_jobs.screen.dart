...
import 'package:posao_app/screens/filter_screen.dart';
...
class _DisplayItJobsScreenState extends State<DisplayItJobsScreen> {
  ...
  bool isQuery = false;
  String lokacija = '', kategorija = '', iskustvo = '';

  void filterData(dynamic data, [bool reset = false]) {
    if(reset) {
      setState(() {
      lokacija = null;
      kategorija = null;
    ...
    setState(() {
      lokacija = data['lokacija'] == null ? '' : data['lokacija'].toString().replaceAll(' ', '-').toLowerCase();
      kategorija = data['kategorija'] == null ? '' : data['kategorija'].toString().toLowerCase();
      iskustvo = data['iskustvo'] == null ? '' : data['iskustvo'].toString().toLowerCase();
      isQuery = true;
    });
  }
        ...
        if (pageNumber != Provider.of<Jobs>(context, listen: false).totPage) {
          try {
            ...
            var res = await Provider.of<Jobs>(context, listen: false)
                .fetchJobsFrom_Dzobs(++pageNumber, isQuery, lokacija, kategorija, iskustvo);
          } catch (error) {}
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: FutureBuilder(
        future:
            Provider.of<Jobs>(context, listen: false).fetchJobsFrom_Dzobs(1, isQuery, lokacija, kategorija, iskustvo),
        builder: (ctx, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return Consumer<Jobs>(
                builder: (ctx, jobsData, child) => jobsData.itJobs.length == 0
                    ? Center(child: Image.asset('assets/nema.gif'))
                    : ListView.builder(
      ...
      floatingActionButton: Row(
        ...
        children: [
          isQuery ? FloatingActionButton.extended(
              onPressed: () => filterData(null, true),
              label: const Text('Poništi filter'),
